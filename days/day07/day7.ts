...
const runNextEvaluations = ({ index, currentRunningTotal, values, total }: { index: number; currentRunningTotal: number; values: Array<number>; total: number; }): Array<PseudoResult> => {
  ...
  return OPERATIONS.map((operation) => {
    ...
    const futureOperations = runNextEvaluations({ index: index + 1, currentRunningTotal: nextRunningTotal, values, total });
